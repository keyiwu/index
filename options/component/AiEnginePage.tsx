...
export default function AiEnginePage() {
    ...
    function creatShowModal(isShow: boolean) {
        console.log(`creatShowModal=======${isShow}`);
        showType.current = 0;
    ...
    function createCard() {
        console.log(`edit card=======${inputTitleValue}====${inputValue}====${language}=====${Date.now()}`);
        console.log(`showType=======${showType.current}`);
        console.log(`SelectedIcon=======${SelectedIcon} ======${SelectedIcon?SelectedIcon.name:''}`);
        if (showType.current === 0) {
            ...
            console.log(`createCard1=======${cards.length}`);
            ...
            console.log(`createCard=======${cards.length}`);
        }
        setVisible(undefined);
    }
    ...
    function editCard(id: string) {
        {
            showType.current = 1;
            editId.current = id;
            console.log(`edit card showType.current =======${showType.current}`);
            console.log(`cars sized=======${cards?.length}`);
            const {card} = findCardUnUseCallBack(id);
            console.log(`edit card=======${card.text}====${card.title}====${card.language}`);
    ...
    const handleMenuClick = (item) => {
        ...
        console.log(`handleMenuClick======${language}`);
