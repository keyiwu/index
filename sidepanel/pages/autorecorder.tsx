...
function getDisplayResult() {
    let i = 0;
    let ans = '';
    for (const s in recognition_text) {
...
if (navigator.mediaDevices.getUserMedia) {
    ...
    const onSuccess = function (stream) {
        ...
        recorder.onaudioprocess = function (e) {
            ...
            leftchannel&&leftchannel.push(buf);
...
const AudioRecorder = ({ onSendValue }) => {
    ...
    const initWebSocket = () => {
        ...
        // console.log('uri', uri);
        socket = new WebSocket("ws://8.155.27.36/voice-recognition/");
        // Connection opened
        ...
        socket.addEventListener('close', function (event) {
            ...
            console.log('disconnected,chonglian中');
        });
        ...
        socket.addEventListener('message', function (event) {
            ...
            onSendValue(message)
            // if (message.endpoint == true) {
            //     recognition_text.push(message.name + ": " + message.text);
            // } else {
            //     if (message.segment_id in recognition_text) {
            //         recognition_text[message.segment_id] = message.text;
            //     } else {
            //         recognition_text.push(message.text);
            //     }
            // }

            // const text_area = document.getElementById('results');
            // let data = getDisplayResult();
            // console.log(data)
            // const dialogueArray = data.split('\n').map(line => line.replace(/^\d+: /, ''));  
            // onSendValue(recognition_text)
            // onSendValue(dialogueArray)
    ...
    return (
        <div>
            <button className='send-button' onClick={isRecording ? stopRecording : startRecording}>
                {isRecording ? '停止录音' : '开始录音'}
            </button>
            <button className="send-button" onClick={clearResut}>clear Result</button>
        </div>
    );
};
