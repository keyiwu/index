...
import {
    ...
    debounce,
    getLatestState,
    MESSAGE_ACTION_SET_QUOTING_CANCEL,// 设置引用退出
    MESSAGE_ACTION_SET_QUOTING_SELECTION_CLEAR_CURSOR,//设置引用清空
    MESSAGE_ACTION_SET_QUOTING_SELECTION_TEXT,//引用文本
    PROMPT_PLACEHOLDER_LANG, // 语言
...
const PromptTags = ({ cards, itemClick,onVisibleTagChange }) => {
    ...
    useEffect(() => {
        console.log('useEffect===checkInViewport');
        checkInViewport();
    ...
    function isElementInViewportX(el: HTMLElement) {
        ...
        console.log('isElementInViewportX',rect.left, rect.right, window.innerWidth || document.documentElement.clientWidth);
        return (
            rect.left >= 0 &&
    ...
    return <div ref={rootRef} className={'flex-row-reverse flex w-full justify-start items-center overflow-hidden hideScrollBar'}>
        {
            cards?.map((item, index) => {
                ...
                console.log(`item.imageKey:==========${item.imageKey}==========${item.imageKey != null}`);
...
// 获取错误，参数 模型，错误，方法，设置当前机器人的返回的消息，激活的tab
const AIError = memo(function ({model, error, start, setCurrentBotResponseMessage, activePage}:
    ...
    useEffect(() => {
        console.log('000000-----',error?.code, model.botName, model.requireLogin);
...
const AITextContent = ({bot, message, pref, i}: {
    ...
    const start = () => {
        if (message.data.prompt) {
            ...
            console.log(`sendMsgInfo:===botName:${bot.botName}===prompt:${prompt}=====fileRefToken:${fileRefToken}====rid:${rid}`);
...
function ConversationContent() {
    ...
    const handleOpenChange = (newOpen: boolean) => {
        ...
        console.log('currentBots==========', currentBots);
        // setIsShowMore(false);
    ...
    async function showQuotingText(title: QuotingType, content: string) {
        ...
        if (title && content) {
            ...
            console.log('titleText', titleText, content);
    ...
    useEffect(() => {
        ...
        document.body.addEventListener('keydown', (e) => {
            if (e.shiftKey && e.metaKey && e.code === 'Enter') {
                goToSearch(e).then(() => {
                    console.log('goToSearchByAskBar is completed');
                }).catch((error) => {
                    console.log('goToSearchByAskBar is Error: ', error);
                });
            }
        });
    ...
    const handleMouseDown = (e: MouseEvent) => {
        console.log(`mousedown handleMouseDown===============${popIsShow}`);
        if (popIsShow) {
            const popComponent = promptPop.current;
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-expect-error
            if (popComponent && !popComponent.contains(e.target)) {
                console.log(`mousedown closePromptPopup=============${popIsShow}`);
    ...
    function handleMessage(message: any) {
        ...
        case MESSAGE_ACTION_SET_QUOTING_CANCEL:
            console.log('MESSAGE_ACTION_SET_QUOTING_CANCEL=========');
    ...
    async function cancelSelectQuot() {
        ...
        console.log('cancelSelectQuot=========',title);
        console.log('cancelSelectQuot quotText[1]=========',quotText[0]);
    ...
    async function textInputFocus() {
        ...
        const isQuotShow = await getLatestState(setIsHaveQuotingText);
        console.log(`textInputFocus=========${isQuotShow}`);
        if (isQuotShow) {
    ...
    function goToAskEngine(msg: string|null, card: any, quotingText?: string, isUploadFile = false, uploadFileInfo: [string, string, Map<string, string>, FileTypes, File | null] | null = null) {
        if (card.id != AskPromptId) {
            console.log(`goToAskEngine===============${msg?msg:'msg==null'}`);
    ...
    function setMsgService(message: IAskAi) {
        console.log('send message by IAskAi:======', JSON.stringify(message));
        console.log('currentBots:', currentBots);
    ...
    async function fileUpload(file: File) {
        ...
        try {
            console.log('uploadResult start=================', conversationId, fileType);
            ...
            console.log('uploadResult finish=================', uploadResult);
            return uploadResult;
        } catch (err) {
            console.log('fileUpload Cancel=================', err.name);
        } finally {
            if (abortHandler) {
            ...
            <div
                ...
                <div className={'w-full h-[23px] flex flex-row-reverse justify-start items-center mb-[8px]'}>
                    ...
                    open={isPromptPopShow} onOpenChange={(visibleAskPop) => {
                        console.log(`visibleAskPop=============${visibleAskPop}`);
                        popIsShow = visibleAskPop;
                        console.log(`popIsShow=============${popIsShow}`);
