...
export default function OpenaiStandaloneAuthWindow() {
    useEffect(() => {
        ...
        void setKeys().then(() => {
            console.trace('ooo openaiAuthKey');
            const {openaiAuthKey, windowId} = getKeys();
            console.trace('ooo openaiAuthKey 1', openaiAuthKey);
            console.trace('ooo windowId 1', windowId);
            console.trace('ooo sessionStorage.getItem(THE_AUTH_KEY)', sessionStorage.getItem(THE_AUTH_KEY));
            if (openaiAuthKey && windowId) {
                interval = setInterval(() => {
                    if (targetSourceValidator()) {
                        clearInterval(interval);
                        console.trace('ooo remove key');
                        resetKeys();

                        console.trace('ooo openaiAuthKey', openaiAuthKey);
                        console.trace('ooo windowId', windowId);
                        ...
                        if (windowId) {
                            console.trace('ooo sendToBackground');
    ...
    const setKeys = async function () {
        ...
        console.trace('ooo setKeys', openaiAuthKey, windowId);
