...
export class CopilotBot extends BotBase implements IBot {
    ...
    async completion({prompt, rid, cb, fileRef, file}: BotCompletionParams): Promise<void> {
        ...
        console.log('file ref', fileSource);

        console.log('this.botSession.getBotConversationId() copilot', this.botSession.session.getBotConversationId());
        ...
        console.trace('copilotConversation', copilotConversation, err);
        ...
        return new Promise((resolve) => {
            const listener = function (message: any) {
                if (message.action === MESSAGE_ACTION_CHAT_PROVIDER_CAPTCHA_SUCCESS) {
                    console.trace('openaiCaptchaValue', copilotCaptchaValue, message.authKey);
